...
public class _10_TypeCasting {
	...
	public static void main(String[] args) {
		...
		// 3. 강제 형변환
		// byte 크기가 큰 타입에서 작은 타입으로 변환 할 때는 타입을 명시해줘야 한다.
		int iNum2 = 60;
		...
		// 덜 세밀한 표현범위를 갖는 타입으로 변환 할 때도 타입을 명시한다.
		float fNum2 = 3.14f;
		long lNum2 = (long)fNum2;
		
		// 4. 연산에서의 형변환
		// 형변환 순서에 따라 연산중에 더 byte크기가 크거나 좀 더 세밀한 표현 범위를 갖는
		// 타입으로 자동 형변환이 일어난다.
		long result1 = lNum2 + iNum2;
		
		// 강제 형변환을 통해서 연산의 결과를 작은 타입으로 변결 할 수도 있다.
		int result2 = (int)lNum2 + iNum2;
		
		int result3 = (int)(lNum2 + iNum2);
		
		// 5. 문자열 결합 연산
		// 문자열 결합 연산에서는 모든 자료형이 문자열로 변환 된 후 연산된다.
		String str = "java";
		int version = 17;
